...
- **UX-DEMO-02 Minimal Living Command Center (DEC-085): РЕАЛИЗОВАН ЛОКАЛЬНО.**
  ...
  следующих решения и три последних сигнала. Источники, основания, сотрудники,
  заказчик и документы раскрываются только по запросу в одной правой панели;
  решение, точный предпросмотр, подтверждение и квитанция собраны в одном
  модальном окне. Контекстный ассистент открывается из единой командной строки
  (`Cmd/Ctrl+K`), отвечает детерминированно на вымышленных данных, показывает
  основания и может открыть нужный профиль или подготовить решение, но не
  выполняет действие. Подтверждённая симуляция по-прежнему честно меняет очередь
  `3/2/7 → 2/2/8`, поднимает новый приоритет и создаёт только синтетическую
  `DEMO-*`/`SIM-*` квитанцию с `externalWrite=false`. После перехода каждый
  следующий ход открывает собственную карточку миссии с владельцем, ущербом,
  нужным решением и точными радарами; основной CTA следует за новым приоритетом,
  а старая квитанция остаётся отдельным вторичным действием. Drawer/modal
  сохраняют исходную точку возврата фокуса, результат решения получает фокус для
  screen reader, reset очищает противоречащий ему чат, а значимый desktop-текст
  больше не ужимается до 8–9 px. Exact-path production gate,
  постоянная демо-маркировка, отсутствие API/provider вызовов, persistence и
  внешних записей из DEC-084 сохранены; визуальная грамматика туров отменена.
  Проверено 2026-07-16: frontend **346/346 passed**, typecheck/lint и production
  build (**17 продуктовых routes**) ✅; backend **674 passed / 1 external
  warning**, Ruff, tracked/untracked demo-secret scan и whitespace checks ✅.
  Независимый финальный аудит не нашёл P1/P2, скрытых network/provider/storage/
  write-путей или оставшихся tour-symbols. Автоматический browser reload после
  перезапуска зависшего старого
  dev-процесса был заблокирован политикой Browser Use, поэтому новый визуальный
