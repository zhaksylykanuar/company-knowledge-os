...
- **UX-DEMO-01 Desktop Product Simulation (DEC-084): РЕАЛИЗОВАНА ЛОКАЛЬНО.**
  Добавлен отдельный desktop-only маршрут `/demo`, который показывает целевой
  FounderOS как завершённый 12-сценный путь: готовый онбординг → четыре радара →
  связанный сигнал → живой штаб → карта отношений → профиль заказчика и
  ключевого лица → команда → документы и основания → сводка → очередь решений
  → подтверждаемый предпросмотр → квитанция → пересчитанные штаб и очередь.
  Это интерактивный продуктовый эталон на полностью вымышленных данных NovaFlow,
  а не доказательство подключённых источников или готовности текущих рабочих
  экранов. Маркировка демо постоянна; прямой вход на финальную сцену показывает
  только ожидаемый формат, а завершённая симуляция меняет очередь с `3/2/7` на
  `2/2/8`, сохраняет синтетическую `DEMO-*`/`SIM-*` квитанцию и не допускает
  повторного выполнения. Есть deep links, история браузера, оглавление,
  просмотренный прогресс, подсказки, ручное исследование, автопоказ с темпом,
  остановка на решении, presenter/fullscreen mode и горячие клавиши. `/demo`
  публичен только как точный путь: в development он доступен локально, а в
  production требует точного `FOUNDEROS_DEMO_ENABLED=true`; переход в рабочий
  `/dashboard` не prefetch-ится. Внутри демо нет API/fetch/provider вызовов,
  форм, cookie/storage persistence или внешних записей. Проверено 2026-07-15:
  frontend **346/346 passed**, typecheck, lint и production build
  (**18 продуктовых маршрутов**) ✅; backend **674 passed / 1 внешнее
  предупреждение** и Ruff ✅; проверки tracked secrets и whitespace ✅;
  browser QA только в заявленном
  desktop-контракте **1280×720** прошёл все 12 сцен без горизонтального или
  внутреннего вертикального overflow, замкнутый цикл, direct-final honesty,
  сохранение прогресса при смене темпа и чистую console error/warn выборку.
  Mobile/tablet адаптация намеренно не выполнялась и не заявляется. Следующий
  локальный продуктовый chunk — переносить утверждённую грамматику из `/demo` в
  реальный computed onboarding и рабочие экраны по одному evidence-backed
  срезу; первый реальный GitHub App read остаётся отдельным human-approved gate.
- **UX-RESET-03 Missions Decision Room (DEC-083): РЕАЛИЗОВАН ЛОКАЛЬНО.**
  ...
  а правая консоль последовательно объясняет причину, последствия, основания,
  доступное решение и записанный результат. Страница загружает один смешанный
  status-window максимум из 100 предложений и фильтрует его локально, поэтому
  верхние метрики честно обозначены как пульс загруженной очереди, а не как
  totals всего workspace. Приоритет активной миссии детерминирован: ошибка →
  ждёт решения → одобрена и готова к preview → одобрена → выполнена → отклонена.
  Evidence, решение, preview и история относятся только к активной миссии;
  смена миссии перемонтирует execution controls и очищает transient confirmation
  state, а pending preview/history/write временно блокирует смену миссии,
  фильтров, workspace и глобальной навигации. Ответы старой workspace не могут
  загрязнить новую очередь; успешный внешний результат остаётся закреплён в
  активной консоли после фонового refresh, а сбой отдельного чтения истории не
  превращает уже подтверждённое выполнение в повторяемую ошибку.
  Bulk review спрятан за отдельной проверкой последствий, ошибки мутаций остаются
  рядом с действием, filters используют `aria-pressed`, а выбор и завершение
  операции переводят keyboard focus в актуальную консоль или empty state.
  Локальные approve/reject, RBAC,
  evidence, receipts и explicit external-write gate сохранены; preview не
  запускается автоматически, потому что его GET фиксирует audit event. Проверено
  2026-07-15: frontend **333/333 passed**, focused Actions **62/62**, typecheck,
  lint и production build (**17 продуктовых routes**) ✅; browser QA при
  **1280×720**, **768×900** и **375×844** не выявил horizontal overflow,
  mobile/tablet metrics не перекрываются, свежих console warnings/errors нет.
  Backend **674 passed / 1 external
  warning** и Ruff ✅; docs checks **8 passed**, tracked-secret и whitespace
  checks ✅. Backend API/schema, DB, provider calls/writes, RBAC и LLM не
  менялись. Следующий локальный UX chunk — сделать onboarding коротким
  визуальным прологом к Штабу; первый реальный GitHub App read остаётся
  отдельным founder-approved внешним gate.
