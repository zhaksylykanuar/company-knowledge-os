#!/usr/bin/env python3
"""Offline private-beta release handoff report for FounderOS.

This command produces a sanitized release handoff packet for the human who will
push, deploy, run read-only smoke, configure GitHub App credentials, and execute
one explicitly approved external-action-result smoke. It does not perform any of
those actions.
...
starts no deploy, performs no external write, and never prints secret values,
credential paths' contents, tokens, database URLs, provider payloads, raw smoke
responses, or internal IDs.

Usage:
    uv run python scripts/private_beta_release_handoff.py [--json]
...
def build_release_handoff(
    *,
    ...
    return {
        "check": "private_beta_release_handoff",
        "offline": True,
        ...
        "recommended_handoff_order": [
            "Push the reviewed branch after explicit human approval.",
            "Deploy through docs/deploy/private-beta.md and docs/deploy/railway-private-beta.md.",
            "Run read-only private-beta smoke via make smoke.",
...
def _print_human(report: dict[str, Any]) -> None:
    ...
    print("FounderOS private-beta release handoff (offline, read-only)")
    ...
    print(f"  local MVP scope complete: {mvp['local_scope_complete']}")
    ...
    print("  recommended handoff order:")
    for index, step in enumerate(report["recommended_handoff_order"], start=1):
        print(f"    {index}. {step}")
    print("  note: this report starts no deploy, provider call, or external write.")
...
def main() -> int:
    ...
    report = build_release_handoff(repos_file=args.repos_file)
    if args.json:
