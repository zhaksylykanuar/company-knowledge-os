#!/usr/bin/env python3
"""Read-only-by-default FounderOS local-runtime smoke checks.

The script reports only step names and HTTP status codes. It never prints API
keys, environment values, response bodies, or provider payloads. The default
path performs GET requests only. ``--include-briefing`` is an explicit local
mutation opt-in that persists a deterministic briefing; it never calls a
provider.
"""
...
def _validate_local_base_url(value: str) -> str:
    """Return a normalized loopback HTTP origin or fail without echoing it."""
    ...
    try:
        ...
        raise SmokeConfigError("local smoke base URL is invalid") from exc
    if (
        parsed.scheme != "http"
        or host is None
        ...
        raise SmokeConfigError("local smoke requires a plain loopback HTTP origin")
    if host.casefold() != "127.0.0.1":
        raise SmokeConfigError(
            "local smoke requires the exact IPv4 listener host 127.0.0.1"
        )
...
def config_from_env_and_args(argv: list[str] | None = None) -> SmokeConfig:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Persist one deterministic local briefing after read-only checks; "
            "this is an explicit local database mutation opt-in."
        ),
    )
    parser.add_argument(
        "--skip-briefing",
    ...
    return SmokeConfig(
        api_base_url=_validate_local_base_url(
            args.api_base_url or "http://127.0.0.1:3000"
        ),
        ...
        include_briefing=args.include_briefing and not args.skip_briefing,
    )
...
def _build_request(step: SmokeStep, config: SmokeConfig) -> Request:
    base_url = _validate_local_base_url(config.api_base_url)
...
def _request_status(
    ...
    config: SmokeConfig,
    *,
    opener: Callable[..., Any] = _NO_REDIRECT_OPEN,
) -> int:
...
def run_smoke(
    ...
    *,
    ...
    for step in READ_ONLY_WORKSPACE_STEPS:
        _run_step(step, config, opener=opener, emit=emit)
    if config.include_briefing:
        for step in OPTIONAL_LOCAL_MUTATION_STEPS:
            _run_step(step, config, opener=opener, emit=emit)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        return 1
    print("FounderOS local-runtime smoke passed")
    return 0
