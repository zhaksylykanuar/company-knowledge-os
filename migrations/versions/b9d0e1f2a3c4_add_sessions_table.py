...
def upgrade() -> None:
    """Create the server-side login sessions table.

    Stores only the token HASH (never the raw token). User.password_hash and
    User.status already exist, so no users-table change is needed in this chunk.
    """
